...
class SelectTemplateStep extends CSelectTemplateWizardStep
{
	...
	function OnPostForm()
	{
		...
		{
			$arTemplates = array("youstore_default");
			...
			if (in_array($templateID,  $arTemplates))
			{
				...
				$wizard->SetVar("templateID", "youstore");
			}
			//else
			//	$wizard->SetVar("templateID", "youstore");
		}
	...
	function ShowStep()
	{
		...
		if (!in_array($defaultTemplateID, $arT)) $defaultTemplateID = "youstore_();";
		$wizard->SetDefaultVar("wizTemplateID", $defaultTemplateID);
        $arTemplateInfo=Array();
        $arThemes=array("default");
        
        foreach($arThemes as $templ){
          $arTemplateInfo['youstore_'.$templ]=array(
                "NAME" => GetMessage("WIZ_TEMPLATE_youstore_".$templ),
                "DESCRIPTION" => "",
                "PREVIEW" => $wizard->GetPath()."/images/ru/small/".$templ.".png",
                "SCREENSHOT" => $wizard->GetPath()."/images/ru/big/".$templ.".png",
            );  
        }
        ...
        */
		$wizard->SetDefaultVars(
			Array(
				...
				//"siteTelephone" => $this->GetFileContent(WIZARD_SITE_PATH."include/footer_contaacts.php", GetMessage("WIZ_COMPANY_TELEPHONE_DEF")),
				"siteCopy" => $this->GetFileContent(WIZARD_SITE_PATH."include/copyright.php", GetMessage("WIZ_COMPANY_COPY_DEF")),
				"shopEmail" => COption::GetOptionString("youstore", "shopEmail", "sale@".$_SERVER["SERVER_NAME"], $siteID),
				...
				//"installyoustoreApp" => COption::GetOptionString("youstore", "installyoustoreApp", "Y", $siteID),	
			)
		);
...
/*		if (file_exists($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/youstoreapp") && LANGUAGE_ID == "ru")
			$this->content .= '
			<div class="wizard-input-form-block">
				'.$this->ShowCheckboxField("installyoustoreApp", "Y", array("id"=>"installyoustoreApp")).
				' <label for="installyoustoreApp">'.GetMessage("wiz_install_youstoreapp").'</label>
			</div>';
*/
		if(LANGUAGE_ID != "ru")
		{
...
class CatalogSettings extends CWizardStep
{
	function InitStep()
	{
		...
		$wizard->SetDefaultVars(
			Array(
				"catalogSubscribe" => (isset($arSubscribe[$siteID])) ? ($arSubscribe[$siteID]['use'] == "Y" ? "Y" : false) : "Y",//COption::GetOptionString("youstore", "catalogSubscribe", "Y", $siteID),
				//"catalogView" => COption::GetOptionString("youstore", "catalogView", "list", $siteID),
...
class ShopSettings extends CWizardStep
{
	function InitStep()
	{
		...
		$wizard->SetDefaultVars(
			Array(
				"shopLocalization" => COption::GetOptionString("youstore", "shopLocalization", "ru", $siteID),
				"shopEmail" => COption::GetOptionString("youstore", "shopEmail", "sale@".$_SERVER["SERVER_NAME"], $siteID),
				"shopOfName" => COption::GetOptionString("youstore", "shopOfName", GetMessage("WIZ_SHOP_OF_NAME_DEF"), $siteID),
				"shopLocation" => COption::GetOptionString("youstore", "shopLocation", GetMessage("WIZ_SHOP_LOCATION_DEF"), $siteID),
				//"shopZip" => 101000,
				"shopAdr" => COption::GetOptionString("youstore", "shopAdr", GetMessage("WIZ_SHOP_ADR_DEF"), $siteID),
				"shopINN" => COption::GetOptionString("youstore", "shopINN", "1234567890", $siteID),
				"shopKPP" => COption::GetOptionString("youstore", "shopKPP", "123456789", $siteID),
				"shopNS" => COption::GetOptionString("youstore", "shopNS", "0000 0000 0000 0000 0000", $siteID),
				"shopBANK" => COption::GetOptionString("youstore", "shopBANK", GetMessage("WIZ_SHOP_BANK_DEF"), $siteID),
				"shopBANKREKV" => COption::GetOptionString("youstore", "shopBANKREKV", GetMessage("WIZ_SHOP_BANKREKV_DEF"), $siteID),
				"shopKS" => COption::GetOptionString("youstore", "shopKS", "30101 810 4 0000 0000225", $siteID),
				"siteStamp" => COption::GetOptionString("youstore", "siteStamp", $siteStamp, $siteID),

				//"shopCompany_ua" => COption::GetOptionString("youstore", "shopCompany_ua", "", $siteID),
				"shopOfName_ua" => COption::GetOptionString("youstore", "shopOfName_ua", GetMessage("WIZ_SHOP_OF_NAME_DEF_UA"), $siteID),
				"shopLocation_ua" => COption::GetOptionString("youstore", "shopLocation_ua", GetMessage("WIZ_SHOP_LOCATION_DEF_UA"), $siteID),
				"shopAdr_ua" => COption::GetOptionString("youstore", "shopAdr_ua", GetMessage("WIZ_SHOP_ADR_DEF_UA"), $siteID),
				"shopEGRPU_ua" =>  COption::GetOptionString("youstore", "shopEGRPU_ua", "", $siteID),
				"shopINN_ua" =>  COption::GetOptionString("youstore", "shopINN_ua", "", $siteID),
				"shopNDS_ua" =>  COption::GetOptionString("youstore", "shopNDS_ua", "", $siteID),
				"shopNS_ua" =>  COption::GetOptionString("youstore", "shopNS_ua", "", $siteID),
				"shopBank_ua" =>  COption::GetOptionString("youstore", "shopBank_ua", "", $siteID),
				"shopMFO_ua" =>  COption::GetOptionString("youstore", "shopMFO_ua", "", $siteID),
				"shopPlace_ua" =>  COption::GetOptionString("youstore", "shopPlace_ua", "", $siteID),
				"shopFIO_ua" =>  COption::GetOptionString("youstore", "shopFIO_ua", "", $siteID),
				"shopTax_ua" =>  COption::GetOptionString("youstore", "shopTax_ua", "", $siteID),

				"installPriceBASE" => COption::GetOptionString("youstore", "installPriceBASE", "Y", $siteID),
			)
		);
	}
	...
	function InitStep()
	{
		...
		if ($shopLocalization == "ua")
			...
			$wizard->SetDefaultVars(
				Array(
					"personType" => Array(
						"fiz" =>  COption::GetOptionString("youstore", "personTypeFiz", "Y", $siteID),
						"ur" => COption::GetOptionString("youstore", "personTypeUr", "Y", $siteID),
					)
				)
			);
	}
...
===*/
	...
	function ShowStep()
	{
		...
		if (
			...
			LANGUAGE_ID == "ru" &&
			(
				COption::GetOptionString("youstore", "wizard_installed", "N", $siteID) != "Y"
...
class DataInstallStep extends CDataInstallWizardStep
{
	...
	{
		if($_SESSION["BX_youstore_LOCATION"] == "Y")
