...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
	...
	while($arElement = $rsElements->Fetch())
	{
		...
		$res = CIBlockElement::GetProperty($arElement["IBLOCK_ID"], $arElement["ID"], array("ID" => "ASC"), array("CODE" => "MORE_PHOTO"));
		if($ob = $res->GetNext())
		{
			$arElement["PROPERTY_MORE_PHOTO_VALUE"] = $ob["VALUE"];
		}
		...
		case "iblock":
			if(array_key_exists($arItem["ITEM_ID"], $arResult["ELEMENTS"]))
			{
				...
				if ($arParams["SHOW_PREVIEW"] == "Y")
				{
					...
					elseif($arElement["PROPERTY_MORE_PHOTO_VALUE"] > 0){
						$arElement["PICTURE"] = CFile::ResizeImageGet($arElement["PROPERTY_MORE_PHOTO_VALUE"], array("width"=>$PREVIEW_WIDTH, "height"=>$PREVIEW_HEIGHT), BX_RESIZE_IMAGE_PROPORTIONAL, true);
					}else{
						$arElement["PICTURE"] = array("src" => SITE_TEMPLATE_PATH."/images/no_photo.png", "width" => 35);
					}
				}
			}
			break;
