	...
	if(!empty($arResult["DELIVERY"]))
	{
		...
		foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery)
		{
			if ($delivery_id !== 0 && intval($delivery_id) <= 0)
			{
				...
				{
					?>
					<div class="item <?=$arProfile["CHECKED"] == "Y" ? "active" : "";?> method-<?=$profile_id?>">
							...
							<a href="javascript:void(0)" for="ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>">
                                <span class="frame">
                                    <?
			...
			else // stores and courier
			{
				...
				?>
					<div class="item <?if ($arDelivery["CHECKED"]=="Y") echo " active ";?> method-<?=$delivery_id?>">
							<input type="radio"
                                class="hidden"
							...
							<a href="javascript:void(0)" for="ID_DELIVERY_ID_<?=$arDelivery["ID"]?>" <?=$clickHandler?>>
                                <span class="frame">
                                    <?
                                    if (count($arDelivery["LOGOTIP"]) > 0):
		...
		echo "<div class='row'><p>Вы не указали адрес доставки, либо доставка в ваш регион не осуществляется.</p></div>";
	}
?>
<div class="clear"></div>
</div>