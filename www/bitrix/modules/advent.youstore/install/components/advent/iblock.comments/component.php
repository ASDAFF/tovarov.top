...
if($arParams["SUCCESS_TEXT"] == '')
    $arParams["SUCCESS_TEXT"] = GetMessage("MF_OK_MESSAGE");
...
if ($this->StartResultCache(36000, false))
{
    ...
    foreach($showBaseFields as $baseField){
        ...
        $arField = array(
            ...
            "SORT" => $sort,
            "TYPE" => "BASE",
            "HINT" => GetMessage("PH_".$msgName),
            "INPUT_TYPE" => (($baseField == "NAME")?"S":"T"),
            "PLACEHOLDER" => GetMessage("PH_".$msgName)
        );
    ...
    foreach($showFields as $field){
        ...
        if(in_array($arField["INPUT_TYPE"], $listTypes, true)){
            ...
            while($rsEnums = $dbEnums->GetNext()){
                $arField["VALUES"][] = array("ID" => $rsEnums["ID"], "NAME" => $rsEnums["VALUE"]);
            }
        }
    ...
    if(check_bitrix_sessid())
    {
        ...
        foreach($arResult["FIELDS"] as $v){
            ...
            if($v["REQUIRED"] && empty($check_v)){
                $arResult["ERROR_MESSAGE"][$v["CODE"]] = str_replace("#NAME#", $v["NAME"], GetMessage("MF_REQ_FIELD"));
            }
        }
        ...
        if($arParams['EMAIL_FIELD'] && strlen($_POST[$arParams['EMAIL_FIELD']]) > 1 && !check_email($_POST[$arParams['EMAIL_FIELD']]))
        ...
        if(empty($arResult["ERROR_MESSAGE"]))
        {
            ...
            $arIBlockFields = array(
                "ACTIVE" => ($arParams["ACTIVATE"] != "Y")?"Y":"N", //set flag ACTIVE from @component -> @arParams
                "IBLOCK_ID" => $iblock_id,
                    ...
                    case "PROPERTY":
                        switch ($f["INPUT_TYPE"]){
                            ...
                            default:
                                $arEventFields["PARAMS"] .= $f["NAME"].": ".$_REQUEST[$f['CODE']]."\n";
                                $arIBlockFields["PROPERTY_VALUES"][$f['CODE']] = prepareIBlockPropertyValue($_REQUEST[$f["CODE"]], $f);
                                //$arIBlockFields["PROPERTY_VALUES"][$f['CODE']] = $_REQUEST[$f['CODE']];
                                break;
                        }
                        break;
                    default:break;
            ...
            //site_log(print_r($arIBlockFields, true));
            ...
            if(!($result = $el->Add($arIBlockFields))){
                ...
                if(($rsIBlock = CIBlockElement::GetIBlockByID($result)) !== FALSE)
                    $arEventFields["EDIT_LINK"] = "http://".$_SERVER["HTTP_HOST"]."/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=".$rsIBlock."&type=request&ID=".$result."&lang=ru&find_section_section=-1&WF=Y";
            }
            ...
            if(empty($arResult["ERROR_MESSAGE"])){
                ...
                $_SESSION["MF_EMAIL"] = htmlspecialcharsbx($_POST["AUTHOR_EMAIL"]);
...
elseif($_REQUEST["success"] == $arResult["PARAMS_HASH"]){
    ...
    $arResult["OK_MESSAGE"] = 'success'/*$arParams["OK_TEXT"]*/;
}
