    ...
    <div class="right">
        ...
        <div class="info-box">
            <div class="price-box">
                ...
                ?>
                <strong class="item_old_price hidden" id="<?=$arItemIDs['OLD_PRICE']?>" style="display: <?=($boolDiscountShow ? 'block !important' : 'none'); ?>"><?=($boolDiscountShow ? number_format(round($arResult['MIN_PRICE']['VALUE']), 0, '.', ' ' ) : ''); ?> <sub><?=GetMessage("CURRENCY_".$arResult["MIN_PRICE"]["CURRENCY"]."_TITLE")?></sub></strong>
                <strong class="item_economy_price hidden" id="<? echo $arItemIDs['DISCOUNT_PRICE']; ?>" style="display: <? echo ($boolDiscountShow ? 'none !important' : 'none'); ?>"><? echo ($boolDiscountShow ? GetMessage('ECONOMY_INFO', array('#ECONOMY#' => $arResult['MIN_PRICE']['DISCOUNT_DIFF'])) : ''); ?></strong>
                <strong class="price-item" id="<? echo $arItemIDs['PRICE']; ?>"><?=number_format(round($arResult['MIN_PRICE']['DISCOUNT_VALUE']), 0, '.', ' ' )?> <sub><?=GetMessage("CURRENCY_".$arResult["MIN_PRICE"]["CURRENCY"]."_TITLE")?></sub></strong>
