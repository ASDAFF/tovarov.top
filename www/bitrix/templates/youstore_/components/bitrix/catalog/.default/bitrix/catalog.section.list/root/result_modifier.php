...
if (0 < $arResult['SECTIONS_COUNT'])
{
	...
	$rsSections = CIBlockSection::GetList(array(), array('IBLOCK_ID' => $arParams["IBLOCK_ID"], 'ID' => array_keys($arMap)), false, array('ID', 'UF_SECTION_ICON'));
	while ($arSection = $rsSections->GetNext())
	{
		if(intval($arSection['UF_SECTION_ICON'])>0){
			if (!isset($arMap[$arSection['ID']]))
				continue;
			$key = $arMap[$arSection['ID']];
			$arResult['SECTIONS'][$key]['UF_SECTION_ICON'] = $arSection['UF_SECTION_ICON'];
		}
	}
}
?>