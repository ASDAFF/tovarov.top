<?
  ...
  /*  for($i=1;$i<11;$i++){
    ...
    foreach($arTabs as &$tab){
        ...
        $tab['TYPE'] = $type;
        foreach($tab['PHOTOS']['VALUE'] as $key => $photo){   //дополнительные фото
    ...
    if(is_array($arResult['DETAIL_PICTURE'])){
        ...
        $file = CFile::ResizeImageGet($arResult['DETAIL_PICTURE'],array('width'=>988,'height'=>332),BX_RESIZE_IMAGE_EXACT,true);
        $arResult['PHOTOS'][]=array_change_key_case($file,CASE_UPPER);
    }
    if(is_array($arResult['PROPERTIES']['MORE_PHOTO']['VALUE'])){
        foreach($arResult['PROPERTIES']['MORE_PHOTO']['VALUE'] as $photo){
            $file = CFile::ResizeImageGet($photo,array('width'=>988,'height'=>332),BX_RESIZE_IMAGE_EXACT,true);
            $arResult['PHOTOS'][]=array_change_key_case($file,CASE_UPPER);
        }
