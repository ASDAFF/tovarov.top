...
<?if(!empty($arResult)):?>
	<ul id="nav">
		...
		foreach($arResult as $arItem):
			...
			if($arItem['DEPTH_LEVEL']==1) $arItem["TEXT"] = strtoupper($arItem["TEXT"]);//."[".$arItem["DEPTH_LEVEL"]."]";  */
		?>
					...
					switch($prevLevel){
						case 3:
							switch($arItem["DEPTH_LEVEL"]){
								case 2: echo "</ul></div><!--.col-->";break;
								case 1: echo "</ul></div><!--.col--></div></div><!--.drop-->";break;
							}
						break;
						case 2: 
							echo "</div></div><!--.drop-->";
		...
		<?
			switch($prevLevel){
				case 3:
					echo "</ul></div><!--.col--></div></div><!--.drop--></li>";break;
				break;
				case 2: 
					echo "</div></div><!--.drop-->";
				break;
