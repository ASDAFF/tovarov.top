...
?>
            ...
            foreach ($arResult['ITEMS'] as $key => $arItem)
            {
                ...
                ?>
                <div id="<? echo $strMainID; ?>" class="product-item mix <?if(!empty($arItem["PROPERTIES"]["DAY_ITEM"]["VALUE"])):?>day-item<?endif?> category-<?=$arItem["PROPERTIES"]["SEX"]["VALUE_ENUM_ID"]?> <?if(!$arItem["CAN_BUY"]):?>unavailable<?endif?>">
                    <div class="holder">
                        <a class="wishlist_delete item-delete" href="javascript:void(0)" id="wl_<?=$strObName?>_remove">x</a>
                        ...
                        <?if(!empty($arItem["PROPERTIES"]["YOUTUBE_VIDEO"]["VALUE"]['TEXT'])):?>
                            <a class="video-link">play</a>
                            <div class="video-content"><?=$arItem['PROPERTIES']['YOUTUBE_VIDEO']['~VALUE']['TEXT']?></div>
                        <?endif?>
                            <div class="image">
                            ...
                            <div class="expand">
                                <div class="expand-holder">
                                    ...
                                    if (!empty($arItem['MIN_PRICE'])/* && $arItem['CAN_BUY']*/)
                                    {
                                        ...
                                        {
                                            ?>
                                            <p class="message"><?=GetMEssage('CATALOG_ECONOM')?> <strong><?=$arItem['MIN_PRICE']["DISCOUNT_DIFF"]?></strong> <sub><?=GetMessage("CURRENCY_".$arItem["MIN_PRICE"]["CURRENCY"]."_TITLE")?></sub>.</p>
                                            <?
                                        }
                                    }?>
                                    <ul class="tools">
                ...
                <?
                    $arJSWLParams = array(
                        "ID" => "wl_".$strObName."_remove",
                        "ELEMENT_ID" => $arItem["ID"],
                        "PARENT_ID" => $arResult["IBLOCK_ID"],
                        "PARENT_TYPE" => "iblock",
                        "AJAX_URL" => "/bitrix/components/brsoft/wishlist.add/templates/.default/ajax.php",
                        "OBJ_ID" => $strMainID
                    );
                ?>
                <script>
                    var wl_<? echo $strObName;?>_remove = new brWishlistRemove(<? echo CUtil::PhpToJSObject($arJSWLParams, false, true); ?>);
                    wl_<?echo $strObName?>_remove.bindEvents();
                </script>
        <?
            }
        ?>
