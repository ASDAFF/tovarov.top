BX.ready(function() {	
	var sku_props = BX.findChildren(BX('basket_form'), {tagName: 'li', className: 'sku_prop'}, true);
	if (!!sku_props && sku_props.length > 0)
	{
		for (i = 0; sku_props.length > i; i++)
		{
			$tagName = sku_props[i].tagName;			
			BX.bind(sku_props[i], 'click', BX.delegate(function(e){ skuPropClickHandler(e, $tagName);}, this));
		}
	}

	function skuPropClickHandler(e, tagName)
	{
		console.log("HANDLE SkuChange");
		if (!e) e = window.event;
		...
		if (!!target && target.hasAttribute('data-value-id'))
		{
			...
			//console.log("basketItemId: "+basketItemId);
			if (!!all_sku_props && all_sku_props.length > 0)
			{
				for (i = 0; all_sku_props.length > i; i++)
				{
					...
					{
						var sku_prop_value = BX.findChildren(BX(all_sku_props[i].id), {tagName: 'li', className: 'active'}, true);
			...
			BX.ajax({
				url: '/bitrix/components/advent/sale.basket.basket/ajax.php',
				method: 'POST',
				data: postData,
				dataType: 'json',
				onsuccess: function(result)
				{
					...
					window.location.reload();//updateBasketTable(basketItemId, result);	
				}
			});
		}
	}
...
{
	recalcBasketCouponAjax();
	//recalcBasket();
}

function recalcBasket(){
    $('#basket_form').attr('action', $('#basket_form').attr('action') + "#basket_form");
	$('#action_var').val('recalculate');
...
function recalcBasketCouponAjax()
{
	BX.showWait();
	...
	BX.ajax({
		...
		{
			BX.closeWait();
			var couponClass = result["VALID_COUPON"]?"valid":"invalid";
			
			var coupon_status = false;
			for (var i in result["BASKET_DATA"].COUPON_LIST) {
				if(result["BASKET_DATA"].COUPON_LIST[i].JS_STATUS=='APPLYED'){
					coupon_status = true;
				}
			}
			if(coupon_status){
			...
			$('#coupon').removeClass("valid").removeClass("invalid").addClass(couponClass);
		}
	});
}
...
function fullBasketReload(){
	//$('.full-basket-reload').click(function(){
		$.get('/ajax/full-basket.php', "", function(html){
			$('.contents .article-container').find('form').remove();
			$('.contents .article-container').append(html);
		});
	//});
}
